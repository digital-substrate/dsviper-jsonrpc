...
export class Session {
    ...
    // ---- DAG navigation
    async heads() {
        return (await this.#call({op: "heads"})).heads;
    }
    ...
    async mergeCommit({label, parent, merged}) {
        return (await this.#call({op: "mergeCommit", label, parent, merged})).commitId;
    }

    async enableCommit({label, parent, enabled}) {
        return (await this.#call({op: "enableCommit", label, parent, enabled})).commitId;
    }

    async disableCommit({label, parent, disabled}) {
        return (await this.#call({op: "disableCommit", label, parent, disabled})).commitId;
    }
