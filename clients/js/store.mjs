...
export const actions = {
  ...
  diff: (attachment, key, value, recursive) => ({ diff: { attachment, key, value, ...(recursive != null && { recursive }) } }),
...
class CommitUndoStack {
  ...
  reset(initialCommitId) { this.#entries = [{ commitId: initialCommitId, disableCommitId: null }]; this.#index = 0; }
  get canUndo() { return this.#index !== 0; }
  get canRedo() { return this.#index < this.#entries.length - 1; }
  undo() { this.#index -= 1; }
  redo() { this.#index += 1; }
  get currentCommitId() { return this.#entries[this.#index].commitId; }
  get currentDisableCommitId() { return this.#entries[this.#index].disableCommitId; }
  set(commitId) {
  ...
  setDisableCommitId(commitId) { this.#entries[this.#index].disableCommitId = commitId; }
}
...
export class CommitStore {
  ...
  collection(attachment) {
    ...
    return {
      ...
      findOne(key) { return store.#session.get(store.#head, attachment, key); },
      keys() { return store.#session.keys(store.#head, attachment); },
      updateOne(key, update, label = "update") { return store.dispatch(toMutations(attachment, key, update), label); },
      insertOne(key, document, label = "insert") { return store.dispatch([{ set: { attachment, key, value: document } }], label); },
    };
  }
  ...
  async reduceHeads() {
    ...
    if (head) { this.#head = head; this.#diverged = false; this.#notify(); }
    return head;
  }
  ...
  async close() { await this.#session.disconnect(); }
}
