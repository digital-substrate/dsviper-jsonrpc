...
await test("subscribe fires on dispatch with the new state", async () => {
  let got = null;
  const off = store.subscribe((s) => { got = s; });
...
await test("undo/redo toggles via the disable-commit (faithful CommitUndoStack)", async () => {
  ...
  await store.undo(); assert.notEqual((await col.findOne(KEY))?.value, 8);  // 1st undo: disableCommit(c8) + record it
  await store.redo(); assert.equal((await col.findOne(KEY)).value, 8);      // redo: disable the disable-commit
  await store.undo(); assert.notEqual((await col.findOne(KEY))?.value, 8);  // 2nd undo: ENABLE the disable-commit (the toggle branch)
  await store.redo(); assert.equal((await col.findOne(KEY)).value, 8);      // redo again
