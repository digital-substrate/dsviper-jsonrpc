...
test("where eq",
     lambda: _eq(vals(run({"attachment": VIS_ID(), "where": {"op": "eq", "path": "value", "value": 2}})["rows"]), [2, 2]))
test("where gte",
     lambda: _eq(vals(run({"attachment": VIS_ID(), "where": {"op": "gte", "path": "value", "value": 3}})["rows"]), [3, 5]))
test("where in",
     lambda: _eq(vals(run({"attachment": VIS_ID(), "where": {"op": "in", "path": "value", "value": [1, 5]}})["rows"]), [1, 5]))
test("where and",
...
test("where not",
     lambda: _eq(vals(run({"attachment": VIS_ID(), "where": {"op": "not", "arg": {"op": "eq", "path": "value", "value": 2}}})["rows"]), [1, 3, 5]))
test("where nested path (color.red gt)",
     lambda: _eq(vals(run({"attachment": VIS_ID(), "where": {"op": "gt", "path": "color.red", "value": 0.25}})["rows"]), [3, 5]))
test("where exists",
     lambda: _eq(len(run({"attachment": VIS_ID(), "where": {"op": "exists", "path": "color", "value": True}})["rows"]), 5))
test("where missing field -> empty",
     lambda: _eq(len(run({"attachment": VIS_ID(), "where": {"op": "eq", "path": "nope", "value": 1}})["rows"]), 0))

test("orderBy asc",
     lambda: _eq([r["document"]["value"] for r in run({"attachment": VIS_ID(), "orderBy": ["value"]})["rows"]], [1, 2, 2, 3, 5]))
test("orderBy desc + limit",
     lambda: _eq([r["document"]["value"] for r in run({"attachment": VIS_ID(), "orderBy": [{"path": "value", "desc": True}], "limit": 2})["rows"]], [5, 3]))
test("skip + limit on ordered",
     lambda: _eq([r["document"]["value"] for r in run({"attachment": VIS_ID(), "orderBy": ["value"], "skip": 1, "limit": 2})["rows"]], [2, 2]))
test("orderBy nested path (position.x) asc + limit",
     lambda: _eq([r["document"]["position"]["x"] for r in run({"attachment": V2D_ID(), "orderBy": ["position.x"], "limit": 3})["rows"]], [10.0, 20.0, 30.0]))

test("select include-list (nested preserved)",
     lambda: _eq(run({"attachment": VIS_ID(), "where": {"op": "eq", "path": "value", "value": 5}, "select": ["value", "color.red"]})["rows"][0]["document"], {"value": 5, "color": {"red": 0.5}}))
test("select alias-object (flat)",
     lambda: _eq(run({"attachment": VIS_ID(), "where": {"op": "eq", "path": "value", "value": 5}, "select": {"v": "value", "r": "color.red"}})["rows"][0]["document"], {"v": 5, "r": 0.5}))
